...
```json
{
  "audit": {
    ...
    "checks": [
      { "id": "credential", "title": "Credential resolved", "status": "pass", "detail": "…" }
    ],
    "ok": true
  },
  "plan": {
...
- **`migrate scopes`** is an _assistant, not a CI gate_ — audit findings are advisory, so a reachability warning or a failing probe still exits `0`. Only real errors fail: `2` usage (a non-numeric `--account`, an unreadable `--from`, or a legacy file with no valid `merchantId`) · `4` config (writing an invalid profile).
