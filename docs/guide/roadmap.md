...
| 10    | v1.0.2+         | Feature surface — `regions`, `accounts`, `datasources`, `notifications`, `quota`, `issues` | 🚀 Ongoing |
...
return-policy sub-resources (v1.0.3–v1.0.6); completing `datasources` with `update` / `fetch`
(v1.0.7); webhook `notifications` subscriptions (v1.0.8); `quota` usage (v1.0.9); and `issues` —
rendered account & product issue resolution (v1.0.10).
